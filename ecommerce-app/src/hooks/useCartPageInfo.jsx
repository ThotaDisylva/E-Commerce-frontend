...
const useCartPageInfo = () => {
    ...
    if (jwtToken) {
      try {
        const response = await axios.put(
          `http://localhost:8080/api/cart/reduce/${cartId}`,
          {},
          {
            headers: {
              Authorization: `Bearer ${jwtToken}`,
    ...
    if (jwtToken) {
      try {
        ...
        if (response.status === 200) {
          ...
          const updatedCartItems = cartItemsInfo?.filter(
  ...
  const cartQuantityChecker = async () => {
    const jwtToken = localStorage.getItem("jwtToken");

    if (jwtToken) {
      try {
        const cartItemsQuantityDetails = cartItemsInfo?.map((item) => ({
          productId: item.productId,
          quantity: item.quantity,
        }));

        const cartItemsQuantityRequest = { cartItemsQuantityDetails };

        const response = await axios.post(
          `http://localhost:8080/api/cart/quantity/check`,
          cartItemsQuantityRequest,
          {
            headers: {
              Authorization: `Bearer ${jwtToken}`,
        ...
        const data = response.data.cartItemsQuantityDetails;
        console.log("quantity info", data);
        console.log("cart info", cartItemsInfo);

        const flag = false;
        const updatedCartItems = cartItemsInfo?.map((cItem) => {
          data?.forEach((item) => {
            if (item.productId === cItem.productId) {
              if (cItem.quantity > item.quantity) {
                flag=true;
                const difference = cItem.quantity - item.quantity;
                cItem.quantity = item.quantity;
                setCartItemCount((prevCount) => prevCount - difference);
                if(cItem.qauntity!==0){
                    toast.info("Item quantity can't exceed stock quantity")
                }else{
                    toast.error("Product Out of Stock, Remove item to proceed")
                }
              }
            }
          });
          return cItem; // Ensure each cItem is returned after potential modification
        });
        console.log("updated onr checking", updatedCartItems);
